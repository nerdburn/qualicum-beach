...
*/
appServices.factory('appModal', ["$document", "$compile", "$rootScope", "$controller", "$timeout", function ($document, $compile, $rootScope, $controller, $timeout) {

    var defaults = {
        modalId: null,
        template: null, // optionally pass in an html string template
        templateUrl: null, // optionally pass in a URL for a template
        controller: null, // just like route controller declaration
        backdrop: true,
        backdropClass: 'ex-overlay',
        modalClass: 'ex-modal',
        showNow: false, // auto show on create
        css: {
            margin: '0 auto'
        }
    };
    ...
    return {
        ...
        create: function(templateUrl, options, passedInScope) {
            
            var self = this;
            
            // templateUrl is optional, so handle arguments if missing
            if(angular.isObject(templateUrl)){
                passedInScope = options;
                options = templateUrl;
            } else {
                options.templateUrl = templateUrl;
            }

            // extend options & replace defaults
            options = angular.extend({}, defaults, options); //options defined in constructor

            var key;
            var idAttr = (options.modalId)? options.modalId : '';

            // create the modal element (jQuery element) and make it hidden
            modalEl = angular.element('<div class="ex-modal ' + options.modalClass + ' off" id="' + options.modalId + '"><div class="ex-modal-body" ng-include="\'' + options.templateUrl + '\'"></div></div>');

            // apply the optional css to modal
            for(key in options.css)
                modalEl.css(key, options.css[key]);

            // create the backdrop (jQuery element)
            backdropEl = angular.element('<div class="'+options.backdropClass +' off"></div>');
            ...
            modalEl.click(function(event){
                var modalBody = $(this).children('.ex-modal-body')[0];
                if($.contains(modalBody, event.target))
                    return false;
                else
                    scope.$close();                    
            });            
            ...
            scope.$open = function() {
                ...
                body.append(modalEl);
                modalEl.removeClass('off').addClass('on');
                if(options.backdrop)
                    backdropEl.removeClass('off').addClass('on');
            }

            scope.$close = function() {
                modalEl.removeClass('on').addClass('off').hide();
                if(options.backdrop)
                    backdropEl.removeClass('on').addClass('off');
                modalEl.remove();
                backdropEl.remove();
            }

            // handle passed in controller
            var ctrl, locals;
            if(options.controller) {
                locals = angular.extend({$scope: scope}, passedInScope);
